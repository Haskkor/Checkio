...
def checkio(text):
    text=text.lower()
    most = {lettre:text.count(lettre) for lettre in text if lettre.isalpha()}
    maxi = max([most[k] for k in most])
    liste = [clef for clef,valeur in most.items() if valeur == maxi]
    liste.sort()
    return liste[0]
